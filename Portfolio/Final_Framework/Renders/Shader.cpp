...
{
	initialStateBlock = Debug_new StateBlock();
	{
...
void Shader::CreateEffect()
{
	...
	if (Path::GetExtension(file) == L"fx")
	{
		...
		if (FAILED(hr))
		{
			...
			{
				std::string str = "";
				str += String::ToString(Path::GetFileName(file)) + std::string("\n");
	...
	{
		std::wstring errorMsg = std::wstring(L"이펙트 파일이 아님 : ") + file;
...
ID3D11InputLayout * Shader::CreateInputLayout(ID3DBlob * fxBlob, D3DX11_EFFECT_SHADER_DESC* effectVsDesc, std::vector<D3D11_SIGNATURE_PARAMETER_DESC>& params)
{
	...
	for (D3D11_SIGNATURE_PARAMETER_DESC& paramDesc : params)
	{
		...
		if (String::StartsWith(name, "INST") == true)
		{
			...
			std::string start = name.substr(0, 1);
...
ID3DX11EffectVariable * Shader::Variable(std::string name)
{
	return effect->GetVariableByName(name.c_str());
}

ID3DX11EffectScalarVariable * Shader::AsScalar(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsScalar();
}

ID3DX11EffectVectorVariable * Shader::AsVector(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsVector();
}

ID3DX11EffectMatrixVariable * Shader::AsMatrix(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsMatrix();
}

ID3DX11EffectStringVariable * Shader::AsString(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsString();
}

ID3DX11EffectShaderResourceVariable * Shader::AsSRV(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsShaderResource();
}

ID3DX11EffectRenderTargetViewVariable * Shader::AsRTV(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsRenderTargetView();
}

ID3DX11EffectDepthStencilViewVariable * Shader::AsDSV(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsDepthStencilView();
}

ID3DX11EffectConstantBuffer * Shader::AsConstantBuffer(std::string name)
{
	return effect->GetConstantBufferByName(name.c_str());
}

ID3DX11EffectShaderVariable * Shader::AsShader(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsShader();
}

ID3DX11EffectBlendVariable * Shader::AsBlend(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsBlend();
}

ID3DX11EffectDepthStencilVariable * Shader::AsDepthStencil(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsDepthStencil();
}

ID3DX11EffectRasterizerVariable * Shader::AsRasterizer(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsRasterizer();
}

ID3DX11EffectSamplerVariable * Shader::AsSampler(std::string name)
{
	return effect->GetVariableByName(name.c_str())->AsSampler();
}

ID3DX11EffectUnorderedAccessViewVariable * Shader::AsUAV(std::string name)
{

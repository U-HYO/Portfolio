...
Texture::Texture(std::wstring file, bool isInTextureFolder, D3DX11_IMAGE_LOAD_INFO* loadInfo)
	: file(file)
{
	if (isInTextureFolder)
	{
...
void Texture::SaveFile(std::wstring file)
{
...
void Texture::SaveFile(std::wstring file, ID3D11Texture2D * src)
{
...
D3D11_TEXTURE2D_DESC Texture::ReadPixel(std::vector<Color>* pixels)
{
	ID3D11Texture2D* srcTexture;
	view->GetResource((ID3D11Resource **)&srcTexture);
...
D3D11_TEXTURE2D_DESC Texture::ReadPixel(ID3D11Texture2D * src, std::vector<Color>* pixels)
{
	D3D11_TEXTURE2D_DESC srcDesc;
	src->GetDesc(&srcDesc);
	...
	UINT* colors = Debug_new UINT[desc.Width * desc.Height];
...
void Textures::Load(Texture * texture, D3DX11_IMAGE_LOAD_INFO * loadInfo)
{
	...
	std::wstring ext = Path::GetExtension(texture->file);
	if (ext == L"tga")
	{
	...
	//TextureDesc exist;	//동일명의 텍스쳐를 로드했을때 건너뜀
	//bool bExist = false;
...
// TextureArray
//-----------------------------------------------------------------------------
TextureArray::TextureArray(std::vector<std::wstring> & names, UINT width, UINT height, UINT mipLevels)
{
...
std::vector<ID3D11Texture2D*> TextureArray::CreateTextures(std::vector<std::wstring>& names, UINT width, UINT height, UINT mipLevels)
{
	std::vector<ID3D11Texture2D *> returnTextures;
	...
	for (UINT index = 0; index < returnTextures.size(); index++)
	{
		...
		TexMetadata metaData;
		std::wstring ext = Path::GetExtension(names[index]);
		if (ext == L"tga")
		{

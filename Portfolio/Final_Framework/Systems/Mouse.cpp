...
void Mouse::Update()
{
	...
	Vector2 centerPos = Vector2(D3D::Width()/2.0f, D3D::Height()/2.0f);
	Vector2 currentMousePos = Vector2((float)point.x, (float)point.y);

	RECT Clip;
	GetClientRect(D3D::GetDesc().Handle, &Clip);
	ClientToScreen(D3D::GetDesc().Handle, (LPPOINT)&Clip);
	ClientToScreen(D3D::GetDesc().Handle, (LPPOINT)(&Clip.right));

	//if (!_KEYCODE->Press(VK_LMENU) && centerPos != currentMousePos)
	//{
	//	SetPosition(D3DXVECTOR3(centerPos.x, centerPos.y, 0));
	//
	//	wheelOldStatus.x = centerPos.x;
	//	wheelOldStatus.y = centerPos.y;
	//
...
LRESULT Mouse::InputProc(UINT _message, WPARAM _wParam, LPARAM _lParam, HWND handle)
{
	if (_message == WM_LBUTTONDOWN || _message == WM_MOUSEMOVE)
	{
		position.x = (float)LOWORD(_lParam);
		position.y = (float)HIWORD(_lParam);
	}

	if (_message == WM_MOUSEWHEEL)
	{
		short tWheelValue = (short)HIWORD(_wParam);
...
void Mouse::SetPosition(D3DXVECTOR3 _position)
{
	POINT point;
	point.x = _position.x;
	point.y = _position.y;

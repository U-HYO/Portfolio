...
WPARAM Window::Run(IExecute * main)
{
	...
	while (true)
	{
		...
		{
			MainRender(); //이곳에서 쓰레드로 로딩화면 구현, 쓰레드영역내에선 Ram<->V-Ram 복사 금지
		}
	}
	mainExecute->Release();
...
{
	Mouse::Get()->InputProc(message, wParam, lParam, handle);
...
void Window::MainRender()
{
	...
	if (ImGui::IsAnyItemHovered() == false && ImGui::IsAnyWindowHovered() == false)
	{
		Keyboard::Get()->Update();
		Mouse::Get()->Update();
	}

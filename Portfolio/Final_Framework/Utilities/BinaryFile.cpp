...
BinaryWriter::BinaryWriter(std::wstring filePath)
...
void BinaryWriter::String(const std::string & data)
{
	UInt(data.size());
...
BinaryReader::BinaryReader(std::wstring filePath)
	: fileHandle(NULL), size(0)
{
	assert(filePath.length() > 0);
	fileHandle = CreateFile

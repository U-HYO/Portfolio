...
void MeshCylinder::BuildTopCap(std::vector<VertexMesh>& vertices, std::vector<UINT>& indices)
{
	float y = 0.5f * height;
	float theta = 2.0f * Math::PI / (float)sliceCount;
...
void MeshCylinder::BuildBottomCap(std::vector<VertexMesh>& vertices, std::vector<UINT>& indices)
{
	float y = -0.5f * height;
	float theta = 2.0f * Math::PI / (float)sliceCount;

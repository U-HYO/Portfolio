	...
	:camera(camera), perspective(perspective)
{
	...
	if(camera==NULL)
		this->camera = Context::Get()->GetCamera(camIndex);
	if (perspective == NULL)
...
void Frustum::GetPlanes(Plane * planes, UINT count)
{
	memcpy(planes, this->planes, sizeof(Plane) * count);
}
...
void Frustum::SetPerspective(Perspective * perspective)
{
